...
final scoreHistoryProvider = FutureProvider.autoDispose
    .family<List<ScoreHistoryEntry>, String>((ref, scenarioId) async {
  // ... (no changes here) ...
...
class _ResultScreenState extends ConsumerState<ResultScreen> {
  ...
  Future<void> _handleShare(
      Map<String, dynamic> scenarioData, Map<String, dynamic> rankData) async {
  ...
  Widget build(BuildContext context) {
    ...
    final scoreToUse = widget.finalScore > widget.previousBest
        ? widget.finalScore
        : widget.previousBest;
                  ...
                  children: [
                    ...
                    const Text('FINAL SCORE',
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    Text('${(widget.finalScore * weightMultiplier).round()}',
                        style: const TextStyle(
                            color: Colors.white,
                            fontSize: 72,
                            fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    Text(
                        'Previous Best: ${(widget.previousBest * weightMultiplier).round()}',
                        style: const TextStyle(
                      ...
                      return subscriptionState.when(
                        ...
                        data: (tier) {
                          if (tier == SubscriptionTier.gold ||
                            ...
                            return PaywallLock(
                              message:
                                  "Upgrade to Gold to track your progress over time.",
                              onTap: () {
            ...
            actions: [
              IconButton(
                ...
                onPressed: _isSharing
                    ? null
                    : () => _handleShare(scenarioData, rankData),
              ),
            ]);
      },
    );
