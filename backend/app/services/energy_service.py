...
def interpolate_energy(score: float, lower: float, upper: float, lower_energy: int, upper_energy: int) -> float:
    if upper == lower:
        return upper_energy
...
def compute_energy_for_lift(score: float, lift: str, standards: dict) -> int:
    ...
    for i in range(len(rank_order) - 1):
        ...
        if lower_val <= score < upper_val:
            return round(interpolate_energy(score, lower_val, upper_val, rank_energy[lower_rank], rank_energy[upper_rank]))
